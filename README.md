...
- Vim helpers: fzf (>= ${FZF_MIN_VERSION:-0.56.0} ensured via binary download if needed), ripgrep, silversearcher-ag, python3 toolchain, LanguageTool (apt if available, otherwise downloaded to /opt/languagetool) + Java runtime, nodejs/npm/yarn (for markdown-preview), and Python packages `pynvim` + `jedi`.
